...
function initSmoothScroll() {
  document.querySelectorAll('a[href^="#"]').forEach((anchor) => {
    anchor.addEventListener("click", function (e) {
      ...
      if (targetElement) {
        const offsetTop = targetElement.offsetTop - config.navHeight + 10;
...
async function loadGitHubData() {
  try {
    const response = await fetch("https://api.github.com/users/osouzacaique");
...
function updateOpenGraphImage(avatarUrl) {
  ...
  const metaTags = [
    ...
    },
    {
      ...
      content: "+6 anos de experiência em PHP, JavaScript e tecnologias cloud",
    },
